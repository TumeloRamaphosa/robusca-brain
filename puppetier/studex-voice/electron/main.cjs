...
const { hermesToolSpecs, HERMES_VOICE_INSTRUCTIONS, executeHermesTool } = require("./hermes-tools.cjs");
...
ipcMain.handle("tools:list", () => [...toolSpecs, ...hermesToolSpecs]);
...
ipcMain.handle("realtime:create-token", async () => {
  ...
  const db = await readDb();
  const instructions = `${HERMES_VOICE_INSTRUCTIONS}\n\n${buildThumbnailBoardInstructions(db)}`;
  ...
  const response = await fetch("https://api.openai.com/v1/realtime/client_secrets", {
    ...
    body: JSON.stringify({
      session: {
        ...
        tool_choice: "auto",
        tools: [...toolSpecs, ...hermesToolSpecs],
        audio: {
          input: {
            turn_detection: {
              type: "semantic_vad",
        ...
        tracing: {
          workflow_name: "StudEx Hermes Voice",
        },
      },
    }),
  });
...
ipcMain.handle("tools:execute", async (_event, toolCall) => {
  ...
  try {
    if (name.startsWith("hermes_")) {
      return await executeHermesTool(name, args);
    }
