...
- 2026-09-08: 🚨 SECOND SECURITY INCIDENT. All 11 repos in the `studex-nexus` constellation are PUBLIC — including `robusca-brain`, which regressed after being flipped private on 2026-06-17. One **live billable key** exposed: SambaNova, in `agents-dr.fixit/ollama-pool/` since 2026-05-11 (verified HTTP 200). Google Gemini key in `The-Nexus-Agents-NEst` was auto-disabled by Google's leak scanner. GitHub PAT + Discord token in robusca-brain history are already dead. Full report: `SECURITY_AUDIT_2026-09-08.md`; actions at top of `KEY_ROTATION_CHECKLIST.md`. **Lesson: June's fix flipped one repo private and stopped there — it treated the symptom. The structural fix is GitHub secret scanning + push protection org-wide, and private-by-default on new repos.**
